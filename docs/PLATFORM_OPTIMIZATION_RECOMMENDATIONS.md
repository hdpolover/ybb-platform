...
| 2026-05-04 | PR #21 - Payment outbox (`ybb-platform/services/api`) | In progress | Adding `payment_outbox_events` table + scheduler publisher behind `ENABLE_PAYMENT_OUTBOX`, with atomic outbox enqueue inside payment state transaction (`payment.succeeded` application/invoice update path). |
