...
| 2026-05-04 | PR #22 - Cursor pagination + streaming exports (`ybb-platform/services/api`) | In progress | Added seek/cursor mode for admin invoice + audit listings (additive `cursor`/`nextCursor` metadata), and switched large Excel exports to batch-streamed generation to avoid loading all rows into memory. |
