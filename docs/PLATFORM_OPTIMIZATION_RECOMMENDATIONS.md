...
| 2026-05-04 | PR #20 - RabbitMQ retry + DLQ (`ybb-platform/services/notification` + `services/api`) | In progress | Added retry/DLQ queue topology bootstrap (`*.retry`, `*.dlq`) and consumer-side retry exhaustion guard that routes exhausted messages to DLQ with retry metadata; payment-event handler failures now rethrow to activate bounded retries. |
