...
| 2026-05-04 | PR #19 - Notification idempotency (`ybb-platform/services/notification`) | In progress | Added Redis-backed event dedupe guard with typed key derivation + TTL policy (`payment=*7d`, default `24h`) and wired all event handlers through idempotency checks; lint for touched files + targeted tests/build are green. |
