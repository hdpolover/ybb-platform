...
| 2026-05-04 | PR #18 - Slow query mitigation for audit + application reads (`ybb-platform/services/api`) | In progress | Optimized audit trail snapshots for `ParticipantApplication`/`ApplicationInvoice`, reduced payload size with sanitization/redaction/truncation before `DataChangeLog.create`, and made audit logging non-blocking in interceptor path. PR: TBD |
