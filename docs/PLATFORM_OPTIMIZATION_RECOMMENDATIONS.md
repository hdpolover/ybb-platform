...
| 2026-05-04 | PR #19 - Notification idempotency (`ybb-platform/services/notification`) | Merged | Added Redis-backed event dedupe guard with typed key derivation + TTL policy (`payment=*7d`, default `24h`) and wired all event handlers through idempotency checks; dedupe observability logs included. PR: https://github.com/hdpolover/ybb-platform/pull/17 |
| 2026-05-04 | PR #20 - RabbitMQ retry + DLQ (`ybb-platform/services/notification` + `services/api`) | Merged | Added retry/DLQ queue topology bootstrap (`*.retry`, `*.dlq`) and consumer-side retry exhaustion guard that routes exhausted messages to DLQ with retry metadata; payment-event handler failures now rethrow to activate bounded retries. PR: https://github.com/hdpolover/ybb-platform/pull/18 |
| 2026-05-04 | PR #21 - Payment outbox (`ybb-platform/services/api`) | Merged | Added `payment_outbox_events` table + scheduler publisher behind `ENABLE_PAYMENT_OUTBOX`, with atomic outbox enqueue inside payment state transaction (`payment.succeeded` application/invoice update path). PR: https://github.com/hdpolover/ybb-platform/pull/19 |
| 2026-05-04 | PR #22 - Cursor pagination + streaming exports (`ybb-platform/services/api`) | In review | Added seek/cursor mode for admin invoice + audit listings (additive `cursor`/`nextCursor` metadata), and switched large Excel exports to batch-streamed generation to avoid loading all rows into memory. PR: https://github.com/hdpolover/ybb-platform/pull/20 |
