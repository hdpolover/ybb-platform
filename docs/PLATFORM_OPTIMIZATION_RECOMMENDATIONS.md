...
| 2026-05-04 | PR #10 - File service security hardening (`ybb-platform/services/file` + `services/api`) | In progress | Adding internal service-key guard for private file endpoints, enforcing user+brand ownership on file-ready flow, and wiring API gateway to derive identity from JWT instead of client-supplied IDs. |
