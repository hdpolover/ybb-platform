...
export default function ProgramRundownsPage() {
  ...
  async function handleDelete() {
    ...
    try { await deleteProgramSchedule(deleteTarget.id); setDeleteTarget(null); fetch(); }
  ...
  return (
    <main className="space-y-4">
      ...
      {showCreate && <ScheduleModal programId={params.programId} onClose={() => setShowCreate(false)} onSaved={fetch} />}
      {editTarget && <ScheduleModal programId={params.programId} item={editTarget} onClose={() => setEditTarget(null)} onSaved={fetch} />}
...
function ScheduleModal({ programId, item, onClose, onSaved }: { programId: string; item?: ProgramSchedule; onClose: () => void; onSaved: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (item) { await updateProgramSchedule(item.id, { title, startTime, endTime: endTime || undefined, location: location || undefined, description: description || undefined }); }
      else { await createProgramSchedule(programId, { title, startTime, endTime: endTime || undefined, location: location || undefined, description: description || undefined }); }
      onSaved(); onClose();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="w-full max-w-md rounded-xl border border-zinc-200 bg-white p-6 shadow-xl">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-sm font-semibold text-zinc-900">{item ? "Edit Session" : "Add Session"}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <Field label="Start Time" required><input required type="text" value={startTime} placeholder="e.g. 09:00" onChange={(e) => setStartTime(e.target.value)} className={inputCls} /></Field>
          <Field label="End Time"><input type="text" value={endTime} placeholder="e.g. 10:30" onChange={(e) => setEndTime(e.target.value)} className={inputCls} /></Field>
