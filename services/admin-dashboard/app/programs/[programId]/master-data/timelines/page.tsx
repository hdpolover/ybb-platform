...
export default function TimelinesPage() {
  ...
  async function handleDelete() {
    ...
    try { await deleteProgramTimelineItem(deleteTarget.id); setDeleteTarget(null); fetch(); }
  ...
  return (
    <main className="space-y-4">
      ...
      {showCreate && <TimelineModal programId={params.programId} onClose={() => setShowCreate(false)} onSaved={fetch} />}
      {editTarget && <TimelineModal programId={params.programId} item={editTarget} onClose={() => setEditTarget(null)} onSaved={fetch} />}
...
function TimelineModal({ programId, item, onClose, onSaved }: { programId: string; item?: ProgramTimeline; onClose: () => void; onSaved: () => void }) {
  const [title, setTitle] = useState(item?.title ?? "");
  const [date, setDate] = useState(item?.date ? item.date.split("T")[0] : "");
  const [type, setType] = useState(item?.type ?? "milestone");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (item) { await updateProgramTimelineItem(item.id, { title, date, type, description, isActive }); }
      else { await createProgramTimelineItem(programId, { title, date, type, description, isActive }); }
      onSaved(); onClose();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="w-full max-w-md rounded-xl border border-zinc-200 bg-white p-6 shadow-xl">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-sm font-semibold text-zinc-900">{item ? "Edit Timeline Item" : "Add Timeline Item"}</h2>
          <button onClick={onClose}><XMarkIcon className="h-5 w-5 text-zinc-400" /></button>
        </div>
        {error && <p className="mb-3 rounded-md bg-red-50 px-3 py-2 text-xs text-red-700">{error}</p>}
        <form onSubmit={handleSubmit} className="space-y-3">
          <Field label="Title" required><input required type="text" value={title} onChange={(e) => setTitle(e.target.value)} className={inputCls} /></Field>
          <Field label="Date" required><input required type="date" value={date} onChange={(e) => setDate(e.target.value)} className={inputCls} /></Field>
          <Field label="Type"><select value={type} onChange={(e) => setType(e.target.value)} className={inputCls}><option value="milestone">Milestone</option><option value="deadline">Deadline</option><option value="event">Event</option><option value="info">Info</option></select></Field>
