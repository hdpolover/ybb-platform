...
interface NationalityDetailsModalProps {
  open: boolean;
  onClose: () => void;
  data: NationalityDatum[];
}

export function NationalityDetailsModal({ open, onClose, data }: NationalityDetailsModalProps) {
  if (!open) return null;
      ...
      >
        ...
        <div className="overflow-hidden rounded-md border border-zinc-200">
          <table className="min-w-full border-collapse text-left text-[11px]">
            ...
            <tbody>
              {data.map((item, index) => {
                const total = data.reduce((sum, n) => sum + n.count, 0);
