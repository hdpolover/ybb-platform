...
import { LightBulbIcon, PencilSquareIcon, TrashIcon, PlusIcon } from "@heroicons/react/24/solid";
...
const mockSubThemes: SubThemeRow[] = [
  {
    id: 1,
    name: "Sustainable Development & Climate Action",
    description: "Youth-led initiatives on SDGs, climate resilience, and green innovation.",
    status: "Active",
  },
  {
    id: 2,
    name: "Innovation, Technology & Future of Work",
    description: "Discussions on digital transformation, AI, and future skills.",
    status: "Active",
  },
  {
    id: 3,
    name: "Cultural Diplomacy & Global Citizenship",
    description: "Exploring cross-cultural collaboration, peace-building, and youth diplomacy.",
    status: "Inactive",
  },
];

export function SubThemesTable() {
  const [rows] = useState<SubThemeRow[]>(mockSubThemes);
  const [showEditModal, setShowEditModal] = useState(false);
  const [selectedRow, setSelectedRow] = useState<SubThemeRow | null>(null);

  const handleEditClick = (row: SubThemeRow) => {
    setSelectedRow(row);
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-start gap-2">
          ...
          <div className="space-y-0.5">
            <h2 className="text-sm font-semibold text-zinc-900 md:text-base">Sub Theme</h2>
            <p className="text-xs text-zinc-500 md:text-sm">Manage the sub themes used in the submission form.</p>
          </div>
        </div>
        <button
          type="button"
        ...
        <table className="min-w-full border-collapse text-left">
          <thead>
            <tr className="border-b border-zinc-200 bg-zinc-50 text-[11px] uppercase tracking-wide text-zinc-500">
              ...
              <th className="px-3 py-2">Sub Theme</th>
        ...
        <div className="fixed inset-0 z-30 flex items-center justify-center bg-black/40 px-3 md:px-4">
          <div className="w-full max-w-md rounded-md border border-zinc-200 bg-white text-xs text-zinc-700 shadow-lg md:text-sm">
            <div className="flex items-center justify-between gap-2 border-b border-zinc-200 px-4 py-2.5">
              <div>
                <h3 className="text-sm font-semibold text-zinc-900">Edit Sub Theme</h3>
                <p className="text-[11px] text-zinc-500">Update sub theme information used in the submission form.</p>
              </div>
              <button
                type="button"
            ...
            <div className="space-y-3 px-4 py-3">
              <div>
                <label className="mb-1 block text-xs font-medium text-zinc-700">Sub Theme</label>
                <input
                  type="text"
