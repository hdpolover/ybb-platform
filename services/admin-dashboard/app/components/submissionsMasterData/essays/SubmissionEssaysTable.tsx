...
import { AddEssayAction, EditEssayAction, DeleteEssayAction } from "./EssayActions";
...
export function SubmissionEssaysTable({ data }: { data: SubmissionEssayRow[] }) {
  return (
              ...
              {data.length === 0 ? (
                ...
                data.map((row, index) => (
                  <tr key={row.id} className="transition-colors hover:bg-zinc-50/50">
                    ...
                    <td className="px-6 py-4 align-top text-zinc-600">{row.wordLimit} words</td>
                    ...
                    <td className="px-6 py-4 align-top text-right">
                      <div className="inline-flex items-center justify-end gap-2">
                        <EditEssayAction essay={row} />
                        <DeleteEssayAction id={row.id} />
                      </div>
                    </td>
                  </tr>
                ))
