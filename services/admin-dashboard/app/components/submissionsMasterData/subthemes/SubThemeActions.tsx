...
export function AddSubThemeAction() {
  ...
  return (
    <>
      ...
      <SubThemeModal isOpen={isOpen} onClose={() => setIsOpen(false)} />
    </>
  );
}

export function EditSubThemeAction({ subTheme }: { subTheme: SubThemeRow }) {
  const [isOpen, setIsOpen] = useState(false);
  return (
    <>
      <button 
      ...
      <SubThemeModal isOpen={isOpen} onClose={() => setIsOpen(false)} initialData={subTheme} />
    </>
  );
}

export function DeleteSubThemeAction({ id }: { id: number }) {
  return (
    <button 
      type="button" 
