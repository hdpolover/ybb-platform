...
/**
 * Vitest suite for the payment-method code generation helper.
 */
...
it("short name produces a code whose shape is unchanged (slug_suffix)", () => {
  const code = buildPaymentMethodCode("Bank BCA");
  assert.match(code, /^bank_bca_[0-9a-z]{1,6}$/);
});

it("an existing code passes through untouched, even if it's long or odd", () => {
...
it("a long name produces a code within the DB column limit", () => {
...
it("truncation never leaves a trailing underscore right before the suffix", () => {
...
it("the real production incident name now produces a valid, in-limit code", () => {
...
it("empty existing code (falsy) still generates a new code rather than passing through", () => {
  const code = buildPaymentMethodCode("Bank BCA", "");
  assert.match(code, /^bank_bca_[0-9a-z]{1,6}$/);
});
