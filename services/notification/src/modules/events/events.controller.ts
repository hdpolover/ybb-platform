...
export class EventsController {
    ...
    if (data.email) {
      ...
      try {
        receiptBuffer = await this.receiptService.generateReceipt({
          ...
          customerName:
            data.metadata?.customer_name || data.customer_name || 'Customer',
  ...
  async handlePaymentFailed(@Payload() data: any, @Ctx() context: RmqContext) {
    ...
    if (data.email) {
      await this.emailService.sendPaymentFailedEmail(data.email, {
        ...
        orderId: data.order_id || data.payment_id,
        reason:
          data.metadata?.failure_reason || 'Transaction could not be processed',
      });
    }
  }
  ...
  async handleUserRegistered(@Payload() data: any) {
    this.logger.log(
      `Received user.registered event: ${JSON.stringify(summarizeEventPayload(data))}`,
    );
  ...
  async handleForgotPassword(@Payload() data: any) {
    this.logger.log(
      `Received user.forgot-password event: ${JSON.stringify(summarizeEventPayload(data))}`,
    );
  ...
  async handleVerifyEmail(@Payload() data: any, @Ctx() context: RmqContext) {
    ...
    try {
      if (data.email && data.token) {
        this.logger.log(
          `Calling emailService.sendVerificationEmail for ${maskEmail(data.email)}`,
        );
        ...
        this.logger.log(
          `Email service returned successfully for ${maskEmail(data.email)}`,
        );
      } else {
        this.logger.warn(
      ...
      this.logger.error(
        `Error processing user.verify-email event: ${error.message}`,
        error.stack,
      );
    }
  ...
  async handleEmailVerified(@Payload() data: any) {
    this.logger.log(
      `Received user.email-verified event: ${JSON.stringify(summarizeEventPayload(data))}`,
    );
