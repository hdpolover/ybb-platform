...
def main():
    ...
    print(f"Declaring Topic Exchange: {EXCHANGE_NAME}")
    channel.exchange_declare(exchange=EXCHANGE_NAME, exchange_type='topic', durable=True)

    for q in QUEUES:
        q_name = q['name']
        print(f"Declaring Queue: {q_name}")
        channel.queue_declare(queue=q_name, durable=True)
        
        for binding_key in q['bindings']:
            print(f"  -> Binding {q_name} to {EXCHANGE_NAME} with key: {binding_key}")
