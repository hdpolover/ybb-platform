...
describe('HomeStrategy', () => {
    ...
    it('caps the homepage program_gallery teaser at 12 while full_gallery carries the whole pool, and prefers the active program\'s own images', async () => {
        const category = {
            id: 'brand-kys', name: 'Korea Youth Summit', bannerUrl: '', websiteUrl: '',
            vision: '', mission: '', metadata: {},
        };
        ...
        // Active program has 15 of its own images and the brand has 1 more from a sibling.
        // The teaser must cap at 12, drawn only from the active program's own pool, while
        // full_gallery keeps all 15 own images (siblings excluded because own pool is non-empty).
        setupMocks();
        const ownImages = Array.from({ length: 15 }, (_, i) => ({
            id: `own-${i}`, type: 'image', imageUrl: `own-${i}.jpg`, title: `Own ${i}`, programId: 'p-kys-4th',
            program: { name: 'Korea Youth Summit 4th', year: 2026 },
        }));
        const siblingImages = [
            { id: 'sib-1', type: 'image', imageUrl: 'sibling-1.jpg', title: 'Sibling 1', programId: 'p-kys-2025',
              program: { name: 'Korea Youth Summit 2025', year: 2025 } },
        ];
        ...
        expect(section?.content.gallery).toHaveLength(12);
        expect(section?.content.images).toHaveLength(12);
        expect(section?.content.full_gallery).toHaveLength(15);
    ...
    describe('program_gallery.tabs (per-edition gallery switcher)', () => {
        const category = {
            id: 'brand-kys', name: 'Korea Youth Summit', bannerUrl: '', websiteUrl: '',
            vision: '', mission: '', metadata: {},
        };
        ...
        it('groups images per edition, orders them newest-year-first and flags the active program as the default tab', async () => {
            setupMocks();
            const images = [
                ...Array.from({ length: 14 }, (_, i) => ({
                    id: `new-${i}`, type: 'image', imageUrl: `new-${i}.jpg`, title: `New ${i}`,
                    programId: 'p-kys-4th', program: { name: 'Korea Youth Summit 4th', year: 2026 },
                })),
                { id: 'old-1', type: 'image', imageUrl: 'old-1.jpg', title: 'Old 1',
                  programId: 'p-kys-2025', program: { name: 'Korea Youth Summit 2025', year: 2025 } },
            ];
            mockPrismaService.programGallery.findMany.mockResolvedValue(images);

            const result: any = await strategy.getData(category as any);
            const tabs = gallerySection(result)?.content.tabs;

            expect(tabs).toHaveLength(2);
            expect(tabs.map((t: any) => t.year)).toEqual([2026, 2025]);
            expect(tabs.map((t: any) => t.program_name)).toEqual([
                'Korea Youth Summit 4th',
                'Korea Youth Summit 2025',
            ]);
            expect(tabs.map((t: any) => t.is_active)).toEqual([true, false]);
            // Each tab is capped like the teaser, and carries only its own edition's images.
            expect(tabs[0].gallery).toHaveLength(12);
            expect(tabs[0].gallery.every((i: any) => i.url.startsWith('new-'))).toBe(true);
            expect(tabs[1].gallery.map((i: any) => i.url)).toEqual(['old-1.jpg']);
        });

        it('emits a single tab (frontend renders no tab bar) when only one edition has images', async () => {
            setupMocks();
            mockPrismaService.programGallery.findMany.mockResolvedValue([
                { id: 'only-1', type: 'image', imageUrl: 'only-1.jpg', title: 'Only 1',
                  programId: 'p-kys-4th', program: { name: 'Korea Youth Summit 4th', year: 2026 } },
            ]);
            ...
            expect(section?.content.tabs).toHaveLength(1);
            expect(section?.content.tabs[0].is_active).toBe(true);
            expect(section?.content.gallery).toHaveLength(1);
        });

        it('gives an edition with zero images no tab at all', async () => {
            setupMocks();
            // The active edition has no uploads yet: it must not produce an empty tab,
            // and the teaser keeps falling back to the brand-wide pool.
            mockPrismaService.programGallery.findMany.mockResolvedValue([
                { id: 'sib-1', type: 'image', imageUrl: 'sibling-1.jpg', title: 'Sibling 1',
                  programId: 'p-kys-2025', program: { name: 'Korea Youth Summit 2025', year: 2025 } },
            ]);
            ...
            expect(section?.content.tabs).toHaveLength(1);
            expect(section?.content.tabs[0].program_id).toBe('p-kys-2025');
            expect(section?.content.tabs.some((t: any) => t.is_active)).toBe(false);
            expect(section?.content.gallery).toHaveLength(1);
        });
    });
